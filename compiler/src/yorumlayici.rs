...
pub use self::toplayan_io::{GuvenliCerezKaydi, ToplayanIo};
...
use crate::intrinsic::{
    self, CSRF_BELIRTECI, DOSYALARI_LISTELE, DOSYA_ATOMIK_TASI, DOSYA_SHA256, DOSYA_SIL,
...
pub trait GirdiCikti {
    ...
    /// Dosyayı aynı dosya sistemi içinde tek görünürlük adımıyla yayımlar.
    fn dosya_atomik_tasi(&mut self, _kaynak: &str, _hedef: &str) -> Result<i64, String> {
        Err("IO adaptörü atomik dosya taşımayı desteklemiyor".into())
    }
    /// Dosyayı siler; bulunmayan dosya idempotent olarak sıfır döndürür.
    fn dosya_sil(&mut self, _yol: &str) -> Result<i64, String> {
        Err("IO adaptörü dosya silmeyi desteklemiyor".into())
    }
    /// Dizindeki normal dosyaların göreli, sıralı yollarını verir.
    fn dosyalari_listele(&mut self, _dizin: &str) -> Result<Vec<String>, String> {
        Err("IO adaptörü dosya listelemeyi desteklemiyor".into())
    }
    /// Dosyayı akış halinde okuyup küçük harfli SHA-256 döndürür.
    fn dosya_sha256(&mut self, _yol: &str) -> Result<String, String> {
        Err("IO adaptörü dosya özetlemeyi desteklemiyor".into())
    }
