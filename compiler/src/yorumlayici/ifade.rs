    ...
    Box::pin(async move {
        let sonuc = match ifade.turu() {
            ...
            Ifade::Intrinsic { kimlik, argumanlar } => {
                ...
                match kimlik.as_str() {
                    ...
                    PAROLA_DOGRULA => {
                        ...
                        son_tarihi_denetle(io, satir)?;
                        Ok(Deger::Mantiksal(io.parola_dogrula(
            ...
            Ifade::ListeSabiti(ogeler) => {
                ...
                for oge in ogeler {
                    degerler
                        .push(degerlendir_async(oge, ortam, program, io, derinlik, satir).await?);
                }
            ...
            Ifade::Ozellik { nesne, ozellik } => {
                ...
                match (ozellik, nesne) {
                    (Ozellik::Adet, Deger::Liste(ogeler)) => {
                        Ok(Deger::TamSayi(ogeler.len() as i64))
                    }
                    (Ozellik::Ilk, Deger::Liste(ogeler)) | (Ozellik::Son, Deger::Liste(ogeler)) => {
                        ...
                        oge.cloned().ok_or_else(|| {
                            Tani::yeni("C007", "Liste boş: ilki/sonu alınamaz.".into(), satir, 1, 1)
                                .onerili(
                                    "Önce \"listenin adedi\" ile boş olup olmadığını kontrol et."
                                        .into(),
                                )
                        })
                    }
                    (Ozellik::Kirpilmis, Deger::Metin(m)) => Ok(Deger::Metin(m.trim().to_string())),
                    (Ozellik::Metni, deger) => metne_sinirli(&deger, satir).map(Deger::Metin),
                    (Ozellik::BinlikliKuruslu, deger @ (Deger::Ondalik(_) | Deger::TamSayi(_))) => {
                    ...
                    (Ozellik::JsonMetin, deger) => json_yaz(&deger, satir).map(Deger::Metin),
            ...
            Ifade::SozlukDegeri { sozluk, anahtar } => {
                let girdiler =
                    match degerlendir_async(sozluk, ortam, program, io, derinlik, satir).await? {
                        Deger::Sozluk(girdiler) => girdiler,
                        _ => return Err(ic_hata(satir)),
                    };
                let anahtar =
                    match degerlendir_async(anahtar, ortam, program, io, derinlik, satir).await? {
                        Deger::Metin(m) => m,
                        _ => return Err(ic_hata(satir)),
                    };
                girdiler
            ...
            Ifade::SozlukteVar {
            ...
            } => {
                {
                    ...
                    let kap =
                        degerlendir_async(sozluk, ortam, program, io, derinlik, satir).await?;
                    if let Deger::Liste(ogeler) = kap {
                        let aranan =
                            degerlendir_async(anahtar, ortam, program, io, derinlik, satir).await?;
            ...
            Ifade::Icerir { metin, aranan } => {
                let metin =
                    match degerlendir_async(metin, ortam, program, io, derinlik, satir).await? {
                        Deger::Metin(m) => m,
                        _ => return Err(ic_hata(satir)),
                    };
                let aranan =
                    match degerlendir_async(aranan, ortam, program, io, derinlik, satir).await? {
                        Deger::Metin(m) => m,
                        _ => return Err(ic_hata(satir)),
                    };
            ...
            Ifade::SureSabiti { milisaniye } => Ok(Deger::Sure {
            ...
            Ifade::GunSonrasi { tarih, miktar } => {
                let (yil, ay, gun) =
                    match degerlendir_async(tarih, ortam, program, io, derinlik, satir).await? {
                        Deger::Tarih { yil, ay, gun } => (yil, ay, gun),
                        _ => return Err(ic_hata(satir)),
                    };
                let miktar = tam_sayi(
                    degerlendir_async(miktar, ortam, program, io, derinlik, satir).await?,
                    satir,
                )?;
            ...
            Ifade::Parcala { metin, ayrac } => {
                let m = metne_sinirli(
                    &degerlendir_async(metin, ortam, program, io, derinlik, satir).await?,
                    satir,
                )?;
                let a = metne_sinirli(
            ...
            Ifade::Degistir { metin, eski, yeni } => {
                ...
                let e = metne_sinirli(
                    &degerlendir_async(eski, ortam, program, io, derinlik, satir).await?,
                    satir,
                )?;
                let y = metne_sinirli(
                    &degerlendir_async(yeni, ortam, program, io, derinlik, satir).await?,
                    satir,
                )?;
                if e.is_empty() {
                ...
                let sonuc_bayti = if y.len() >= e.len() {
                    m.len()
                        .checked_add(eslesme.checked_mul(y.len() - e.len()).ok_or_else(|| {
                            deger_kaynak_tanisi(
                                satir,
                    ...
                    m.len()
                        .checked_sub(eslesme.saturating_mul(e.len() - y.len()))
                }
                .ok_or_else(|| {
                    deger_kaynak_tanisi(
                        satir,
            ...
            Ifade::Rastgele { alt, ust } => {
                let alt = tam_sayi(
                    degerlendir_async(alt, ortam, program, io, derinlik, satir).await?,
                    satir,
                )?;
                let ust = tam_sayi(
                    degerlendir_async(ust, ortam, program, io, derinlik, satir).await?,
                    satir,
                )?;
                if alt > ust {
                    return Err(Tani::yeni(
                        "C006",
                        format!(
                            "Rastgele aralığı ters: {} ile {} arasında sayı üretilemez.",
                            alt, ust
                        ),
                        satir,
                        1,
            ...
            Ifade::Degisken { cozulmus, ham, .. } => {
                let ad = program
                    ...
                    .ok_or_else(|| ic_hata(satir))?;
                ortam.get(&ad).cloned().ok_or_else(|| {
                    Tani::yeni(
                        "C001",
                        format!("\"{}\" için değer bulunamadı.", ham),
                        satir,
                        1,
                        1,
                    )
            ...
            Ifade::Degil(ic) => {
                let deger = mantiksal(
                    degerlendir_async(ic, ortam, program, io, derinlik, satir).await?,
                    satir,
                )?;
            ...
            Ifade::IslemCagrisi {
                ...
                for arg in argumanlar {
                    degerler
                        .push(degerlendir_async(arg, ortam, program, io, derinlik, satir).await?);
                }
                islem_cagir(ifade, islem_adi, degerler, program, io, derinlik + 1, satir)
                    .await?
                    .ok_or_else(|| ic_hata(satir))
            }
            Ifade::SayiyiDene(ic) => {
