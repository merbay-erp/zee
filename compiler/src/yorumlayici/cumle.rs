        ...
        for cumle in cumleler {
            ...
            match cumle {
                ...
                Cumle::Sor { istem, satir } => {
                    let istem_degeri =
                        degerlendir_async(istem, ortam, program, cikti, derinlik, *satir).await?;
                ...
                Cumle::KezTekrarla { adet, govde, satir } => {
                    let adet = tam_sayi(
                        degerlendir_async(adet, ortam, program, cikti, derinlik, *satir).await?,
                        *satir,
                    )?;
                    let kapsam = kapsam_baslat(ortam);
                    for _ in 0..adet.max(0) {
                        if let Akis::Don(d) =
                            blok_calistir_async(govde, ortam, program, cikti, derinlik).await?
                        {
                ...
                Cumle::AralikDongusu {
                    ...
                    let bastan = tam_sayi(
                        degerlendir_async(bastan, ortam, program, cikti, derinlik, *satir).await?,
                        *satir,
                    )?;
                    let sona = tam_sayi(
                        degerlendir_async(sona, ortam, program, cikti, derinlik, *satir).await?,
                        *satir,
                    )?;
                    let kapsam = kapsam_baslat(ortam);
                ...
                Cumle::OlduguSurece {
                    ...
                    loop {
                        let devam = mantiksal(
                            degerlendir_async(kosul, ortam, program, cikti, derinlik, *satir)
                                .await?,
                            *satir,
                        )?;
                        if !devam {
                ...
                Cumle::Ise {
                    ...
                    for kol in kollar {
                        if mantiksal(
                            degerlendir_async(&kol.kosul, ortam, program, cikti, derinlik, *satir)
                                .await?,
                            *satir,
                        )? {
                            let kapsam = kapsam_baslat(ortam);
                            if let Akis::Don(d) =
                                blok_calistir_async(&kol.govde, ortam, program, cikti, derinlik)
                                    .await?
                            {
                                return Ok(Akis::Don(d));
                            }
                ...
                Cumle::HerBiri {
                    ...
                    for (sira, oge) in ogeler.into_iter().enumerate() {
                        ...
                        let akis =
                            blok_calistir_async(govde, ortam, program, cikti, derinlik).await?;
                        if liste_mi {
                            gezme_ogesini_geri_yaz(ortam, kaynak_adi.as_deref(), ad, sira);
                        }
                ...
                Cumle::Gore {
                    ...
                    let konu =
                        degerlendir_async(konu, ortam, program, cikti, derinlik, *satir).await?;
                    let mut eslesti = false;
                    for (deger, govde) in kollar {
                        let deger =
                            degerlendir_async(deger, ortam, program, cikti, derinlik, *satir)
                                .await?;
                        if deger == konu {
                            let kapsam = kapsam_baslat(ortam);
                            if let Akis::Don(d) =
                ...
                Cumle::SunucuBaslat { kapi, satir } => {
                    let kapi = tam_sayi(
                        degerlendir_async(kapi, ortam, program, cikti, derinlik, *satir).await?,
                        *satir,
                    )?;
                    son_tarihi_denetle(cikti, *satir)?;
                ...
                Cumle::Yonlendir { adres, satir } => {
                    let hedef = metne_sinirli(
                        &degerlendir_async(adres, ortam, program, cikti, derinlik, *satir).await?,
                        *satir,
                    )?;
                    son_tarihi_denetle(cikti, *satir)?;
                ...
                Cumle::CerezSil { ad, satir } => {
                    let ad = metne_sinirli(
                        &degerlendir_async(ad, ortam, program, cikti, derinlik, *satir).await?,
                        *satir,
                    )?;
                    son_tarihi_denetle(cikti, *satir)?;
                ...
                Cumle::OturumAc {
                    ...
                    let kullanici = metne_sinirli(
                        &degerlendir_async(kullanici, ortam, program, cikti, derinlik, *satir)
                            .await?,
                        *satir,
                    )?;
                    let rol = metne_sinirli(
                        &degerlendir_async(rol, ortam, program, cikti, derinlik, *satir).await?,
                        *satir,
                    )?;
                    son_tarihi_denetle(cikti, *satir)?;
                ...
                Cumle::IcindeBlogu {
                    ...
                    let sure_ms =
                        match degerlendir_async(sure, ortam, program, cikti, derinlik, *satir)
                            .await?
                        {
                    ...
                    match sonuc {
                        ...
                        Err(tani) if bu_son_tarihin_iptali(&tani, kimlik) => {
                            if let Some(blok) = yetismezse {
                                let kapsam = kapsam_baslat(ortam);
                                if let Akis::Don(d) =
                                    blok_calistir_async(blok, ortam, program, cikti, derinlik)
                                        .await?
                                {
                                    return Ok(Akis::Don(d));
                                }
                ...
                Cumle::ProgramiBitir { kod, satir } => {
                    ...
                    let kod = match kod {
                        Some(ifade) => {
                            let deger = tam_sayi(
                                degerlendir_async(ifade, ortam, program, cikti, derinlik, *satir)
                                    .await?,
                                *satir,
                            )?;
                ...
                Cumle::Olmali { kosul, satir } => {
                    ...
                    let (tuttu, detay) = match kosul.turu() {
                        Ifade::Karsilastirma { sol, sag, .. } => {
                            let sol_deger =
                                degerlendir_async(sol, ortam, program, cikti, derinlik, *satir)
                                    .await?;
                            let sag_deger =
                                degerlendir_async(sag, ortam, program, cikti, derinlik, *satir)
                                    .await?;
                            let sonuc = mantiksal(
                                degerlendir_async(kosul, ortam, program, cikti, derinlik, *satir)
                                    .await?,
                ...
                Cumle::HataDondur {
                    ...
                    let mesaj =
                        degerlendir_async(mesaj, ortam, program, cikti, derinlik, *satir).await?;
                    ...
                    let neden = match neden {
                        Some(ifade) => {
                            match degerlendir_async(ifade, ortam, program, cikti, derinlik, *satir)
                                .await?
                            {
                ...
                Cumle::BolVeAta {
                    ...
                    let pay =
                        degerlendir_async(pay, ortam, program, cikti, derinlik, *satir).await?;
                    let payda =
                        degerlendir_async(payda, ortam, program, cikti, derinlik, *satir).await?;
                ...
                Cumle::CagriCumlesi { cagri, satir } => {
                    ...
                    } = cagri.turu()
                    {
                        ...
                        for arg in argumanlar {
                            degerler.push(
                                degerlendir_async(arg, ortam, program, cikti, derinlik, *satir)
                                    .await?,
                            );
                        }
                        islem_cagir(
                ...
                Cumle::DosyayaYaz {
                    ...
                    let yol = match degerlendir_async(yol, ortam, program, cikti, derinlik, *satir)
                        .await?
                    {
                        Deger::Metin(m) => m,
                        _ => return Err(ic_hata(*satir)),
                    ...
                    let icerik_degeri =
                        degerlendir_async(icerik, ortam, program, cikti, derinlik, *satir).await?;
                    let icerik = metne_sinirli(&icerik_degeri, *satir)?;
                    son_tarihi_denetle(cikti, *satir)?;
                    cikti.dosya_yaz(&yol, &icerik, *ekleme).map_err(|hata| {
                        Tani::yeni(
                            "C013",
                            format!("Dosyaya yazılamadı: {}.", hata),
                            *satir,
                            1,
                            1,
                        )
                ...
                Cumle::SozlukAta {
                    ...
                    let ad = program
                        ...
                        .ok_or_else(|| ic_hata(*satir))?;
                    let anahtar =
                        match degerlendir_async(anahtar, ortam, program, cikti, derinlik, *satir)
                            .await?
                        {
                            Deger::Metin(m) => m,
                            _ => return Err(ic_hata(*satir)),
