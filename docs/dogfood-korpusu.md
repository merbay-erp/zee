...
- `kip`: `denetle` (ürün politikasıyla derleme+yetkinlik) ya da `calistir`
  (hermetik IO ile yürütme).
...
- `dosya`: etkin ürün kökü altında `.dil`; kanonik biçimde ve sürtünmeyi
  anlatan `# ` yorumuyla başlar. `not` en az 20 karakter.
...
- Girmez: dil tasarımı örneği (golden), düzeltilmiş compiler bug'ı
  (`regression/`), ürün kaynağının kopyası.
