...
| LSP | `test:lsp_testi`<br>`test:olcum_lsp_process_testi`<br>`lib:lsp::cikti::testler::`<br>`lib:lsp::kaynak_siniri_testleri::`<br>`bin:dillsp:testler::` | JSON-RPC framing tani hover completion definition rename ve gercek process stdio cold-start olcumu. |
