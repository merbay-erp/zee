...
| IO | `compiler/tests/deterministik_io_profili_testi.rs`<br>`compiler/tests/io_izi_testi.rs`<br>`compiler/tests/postgresql_testi.rs`<br>`compiler/tests/fixtures/postgresql-recovery-v1.tsv` | — | — | `eszamanlilik`<br>`web_guvenlik`<br>`cli`<br>`proje`<br>`uctan_uca` |
...
| IO | `test:deterministik_io_profili_testi`<br>`test:io_izi_testi`<br>`test:postgresql_testi`<br>`lib:ag_istemcisi::testler::`<br>`lib:kalici_dosya::tests::`<br>`lib:postgresql::testler::` | — | Deterministik profil, replay, ag yaniti, PostgreSQL parametre/recovery/Sonuc siniri ve atomik kalici dosya. |
