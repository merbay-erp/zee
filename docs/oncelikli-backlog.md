...
| K-163 | **DEVAM EDİYOR** | Çatlı 509 Zee satırında PostgreSQL saha kanıtını geçti. Transaction dışı read tek reconnect ile iyileşiyor. F027 gerçek backend-kill provasında başarısız write'ı tekrarlamadan 503 verdi, worker'ı yaşattı; sonraki GET yeni PID ile ve sonraki bağımsız write 303 ile geçti. Gerçek TLS/pool, wire-level COMMIT ambiguity, medya ve 1000+ LOC skor kartı bekleniyor |
