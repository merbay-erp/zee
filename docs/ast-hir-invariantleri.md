...
|---|---|---|
| Parser sonrası | `AyristirilmisAst` | Semantic bağlar boş; her ifade doğrudan tek kesin kaynak zarfında; AST biçimi mümkün |
| Checker sonrası | `BaglanmisProgram` / `HirProgram` | Her AST ifadesi tek ve eksiksiz typed HIR kaydına, AST ile aynı kesin aralığa sahip |
...
yakalar. K-126 ile spansiz ya da iç içe `Ifade::Kaynakli` zarfı da parser
fazında geçersizdir.
...
- HIR kesin aralığının AST'deki `AstKaynakAraligi` ile birebir eşleşmesi,
...
2. Parser'da düğümü ve bütün alt ifadelerini tam token bölgesiyle ayrı ayrı
   kaynaklandır.
3. Checker-fazında beklenen `HirBagi` ve değer türünü tanımla.
4. Bütün alt ifadeleri yapısal yol üreterek ziyaret et.
5. İmkânsız sentetik biçimleri fail-closed kontrol et.
6. Olumlu uçtan uca ve en az bir bozuk AST/HIR regresyonu ekle.
7. Bu rehberi, typed HIR rehberini ve ilgili ADR/backlog/sürüm kapısını aynı
   değişiklikte güncelle.

## Kanıt

- `tests/invariant_testi.rs`: geçerli parsed+bound hat, parser semantic bağ
  sızıntısı, spansiz ifade ve imkânsız AST biçimleri.
